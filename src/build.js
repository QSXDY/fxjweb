...
function buildDist() {
  ...
  /* 套餐二级页静态化：dist/<slug>/index.html */
  const s = d.settings || {};
  const pl = (s.planLines && typeof s.planLines === 'object') ? s.planLines : {};
  Object.keys(pl).forEach(function (slug) {
    if (!/^[a-z0-9][a-z0-9-]{0,49}$/.test(slug)) return;
    const html = render.renderPlanLineCached(pl[slug]);
    const outDir = path.join(DIST_DIR, slug);
    fs.mkdirSync(outDir, { recursive: true });
    fs.writeFileSync(path.join(outDir, 'index.html'), html, 'utf8');
    console.log('[build] dist/' + slug + '/index.html（套餐页）');
  });
